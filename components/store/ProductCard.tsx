...
import { preorderOpen } from '@/lib/catalog/queries';
...
export function ProductCard({ orgSlug, product }: { orgSlug: string; product: ProductView }) {
  ...
  const preClosed = isPre && !preorderOpen(product);
  return (
      ...
      <Card className="h-full overflow-hidden transition-shadow group-hover:shadow-md">
        ...
        <CardContent className="grid gap-1 p-4">
          <div className="flex items-start justify-between gap-2">
            ...
            {isPre && (
              <Badge variant={preClosed ? 'outline' : 'secondary'}>
                {preClosed ? 'Pre-order closed' : 'Pre-order'}
              </Badge>
            )}
